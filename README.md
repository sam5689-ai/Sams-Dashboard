...
- **Read** — click a conversation to open it full-page (not a popup) and full-width on desktop, same as clicking into a conversation in Gmail itself; an icon-only back arrow returns you to the list. Marks every message in it read in Gmail too, just like opening it there would.
- **Threaded conversation view** — every message in the conversation is shown stacked in order, collapsed to a one-line header (sender + snippet) except the most recent, which is expanded; click any header to expand/collapse it, matching Gmail's own conversation view. The email body itself sits flush on the page with no boxed border, same as Gmail's reading pane.
- **Toolbar + pagination** — Archive, Delete, and Mark as unread live in a persistent icon toolbar above the conversation (next to the back arrow), and a "1 of N" counter with prev/next arrows lets you step through the current list without going back to the inbox each time (loads the next page automatically if you page past what's already loaded).
- **Subject tags** — a small "Inbox" pill (plus Social/Promotions/Updates/Forums if applicable) shown next to the subject, mirroring Gmail's own conversation tags.
- **Star, reply, more** — each message's row has a star toggle (a real Gmail star — `is:starred` works as a tab query), a quick-reply icon, and a **⋮** menu with **Reply all** and **Mark as unread**.
- **Expandable "to" line** — click the "to me ▾" line under a sender's name to reveal the full From/To/Cc/Date detail, collapsed by default the way Gmail's own reader works.
...
- **Reply / Reply All / Forward** — Reply and Forward are pill buttons at the bottom of the conversation; Reply All lives in each message's **⋮** menu. Each opens a compose modal prefilled with the right recipients, subject, and quoted original, and sends via Gmail so it threads correctly in the real conversation (using every message's Message-ID in the thread for proper `References`).
- **Compose** — the toolbar button starts a brand-new email from scratch.
- **Archive / Delete** — available both on each row and in the reader toolbar; both act on the whole conversation at once (all its messages), the same as archiving/deleting a conversation in Gmail.
- **Turn into a task** — the checkmark button on each row applies the `ToDashboard` label and immediately runs the email-to-task sync on it. A green checkmark means that email is already queued.
